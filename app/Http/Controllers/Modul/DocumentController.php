...
class DocumentController extends Controller
{
    ...
    public function upload(Request $request){
        ...
        if ($request->file("upload") != null) 
        {
            ...
            $file_size = $this->Functions->formatBytes($file->getSize());
    ...
    public function getData(Request $request, $regid = null)
    {
        ...
        $select = [
            'tr_document.*',
            DB::RAW('CONCAT(nama_file, ".", tipe_file) AS nm_file'),
        ];
        $table = DB::table("tr_document");
        $table->where('regid', empty($regid) ? $request->regid : $regid);
        $query = $this->query->generateQuery($request, $table, $columns, $select);
        // return $query;
        $data = array();
        foreach ($query[0] as $row) {
            // if (in_array($session->level, ['maker', 'checker', 'approver'])) {
            ...
            //                     </a>';
            // }
            $nestedData[] = "<a href='" . url("public/" . $row->file) . "?t=" . date('Y-m-d_h:m:s') . "' target='_blank'>" . $row->nm_file . "&nbsp;<i class='bi bi-box-arrow-up-right'></i></a>";
            // $nestedData[] = $row->nama_file;
            $nestedData[] = $row->tglupload;
            $nestedData[] = $row->ukuran_file;
