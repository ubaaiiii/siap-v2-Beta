...
class RevisiController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        // {
                                ...
                                '<div class="card border-0">'.
                                    '<div class="card-body">'.
                                        '<div class="row container mb-2">'.
                                            ...
                                            '</div>'.
                                        '</div>'.
                                        '<a href="'.'revisi/add/'. Crypt::encryptString($p->regid).'" class="btn btn-default w-100 shadow small">Revisi</a>'.
                                    '</div>'.
                                '</div>'.
                            '</div> ';
                }
            ...
            //         ->make(true);
        // }
        // return view('master.revisi.index');
    }
    public function getDesktop(Request $request)
    {
        if($request->ajax())
        {
            ...
            $sTable = "SELECT regrev,regid,tglrevisi,jnsrev,createby,dataawal,dataakhir,comment,aksi FROM (SELECT t.regrev, t.regid, t.tglrevisi, mr.msdesc jnsrev, t.createby, t.dataawal, t.dataakhir, t.comment, t.regid aksi FROM tr_sppa_revisi t LEFT JOIN ms_master mr on mr.msid = t.jnsrev and mr.mstype='REVISI') t_baru";
            $sTable .= " WHERE regrev LIKE '$cari' OR regid LIKE '$cari' OR tglrevisi LIKE '$cari' OR jnsrev LIKE '$cari' OR createby LIKE '$cari' OR dataawal LIKE '$cari' OR dataakhir LIKE '$cari' OR comment LIKE '$cari' OR aksi LIKE '$cari' LIMIT 100";
        	// $sTable .= "  LIMIT 100";
            $data = DB::select($sTable);
            // dd($data);
            return DataTables::of($data)
                ->addColumn('action', function($data){
                    $button = '<a href="'.'revisi/add/'. Crypt::encryptString($data->regid).'"  class="btn btn-default btn-sm" style="display:inline !important;">Revisi</a>&nbsp;';
                    return $button;
                })
                ->rawColumns(['action'])
                ->make(true);
    ...
    // dd($result);
        ...
        $agent = new \Jenssegers\Agent\Agent;
            return view('modul.revisi.index_desktop', compact('data','revisi'));
        // if($agent->isDesktop()){
        // }else{
        //     return view('modul.revisi.index', compact('data','revisi'));
        // }
    }
    public function add($id)
    {
        ...
        $data = DB::select("SELECT aa.regid,aa.noktp,aa.nama,aa.noktp,aa.jkel,aa.pekerjaan,aa.cabang,aa.tgllahir,aa.mulai,aa.akhir,aa.masa,aa.up,aa.status,aa.createdt,aa.createby,aa.editdt,aa.editby,aa.validby,aa.validdt,aa.nopeserta,aa.usia,aa.premi,aa.epremi,aa.tpremi,aa.bunga,aa.tunggakan,aa.produk,aa.mitra,aa.asuransi,aa.policyno, ab.tglrevisi,ab.comment,ab.jnsrev,ab.regrev,ab.dataawal,ab.dataakhir FROM tr_sppa aa LEFT JOIN tr_sppa_revisi ab ON aa.regid = ab.regid WHERE  aa.regid = '$sid'");
        ...
        $cab = DB::select("select   ms.msid comtabid,msdesc comtab_nm from ms_master ms where ms.mstype='cab'  order by ms.mstype");
        $kerja = DB::select("select   ms.msid comtabid,msdesc comtab_nm from ms_master ms where ms.mstype='kerja'  order by ms.mstype");
        $jnsrev=DB::select("select  ms.msid comtabid,msdesc comtab_nm from ms_master ms where ms.mstype='revisi'  order by ms.mstype ");
        // dd(DB::select('select * from tr_sppa_revisi'));
        if(empty($data[0]->regid)){
            return redirect()->intended('revisi')->with('error', 'Data Tidak dapat ditemukan');
        }
        return view('modul.revisi.add', compact('data','produk','jkel','mitra','cab','kerja','jnsrev'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $sqlRev     = mysql_query("SELECT * FROM ms_master WHERE `mstype`='revisi' AND `msid`='$sjnsrev'");
        $fetchRev   = mysql_fetch_array($sqlRev);
        $awal       = $fetchRev['editby']."-sebelumnya";
        $dataAwal   = $_POST[$fetchRev['editby']."-sebelumnya"];
        $dataAkhir  = $_POST[$fetchRev['editby']];
        
        $sqlDataRev = mysql_query("SELECT * FROM ms_master WHERE `msid` = '$sjnsrev'");
        $fetchDaRev = mysql_fetch_array($sqlDataRev);
        $dataRevisi = $fetchDaRev['msdesc'];
        
        if (!in_array($sjnsrev, array('R1','R6','R7'))) {   // Nama, No Pinjaman, No KTP
            
            $sqlDataAwal= mysql_query("SELECT * FROM ms_master WHERE `msid` = '$dataAwal'");
            $sqlDataAkhir= mysql_query("SELECT * FROM ms_master WHERE `msid`= '$dataAkhir'");
            $fetchAwal  = mysql_fetch_array($sqlDataAwal);
            $fetchAkhir = mysql_fetch_array($sqlDataAkhir);
        ...
        $insertRevisi   = mysql_query("INSERT INTO tr_sppa_revisi 
        ...
        $insertLog      = mysql_query("INSERT INTO tr_sppa_log
                                                   (regid, status, createdt, createby, comment) 
                                       SELECT '$sregid', '14', '$sdate', '$userid', '$komen'");
        
        $updateData     = mysql_query("UPDATE tr_sppa
        ...
        return redirect()->intended('revisi')->with('success', 'Data Berhasil ditambahkan');
    }
    public function edit($id)
    {
        ...
        return view('master.revisi.edit', compact('data','produk','jkel','kerja','cab','mitra','hubungan','dokumen','file','sid'));
    }
    public function approve($id){
        ...
        return redirect()->intended('revisi')->with('success', 'Data Berhasil disimpan');
    }
    public function log($id)
    {
        ...
        // dd($data2);
        return view('master.revisi.log', compact('data','data2'));
    }
    public function report($id)
    {
        // dd(Session::get('login')[0]->id_member);
        $regid = Crypt::decryptString($id);
        return view('master.revisi.report', compact('regid'));
    ...
    public function reportlpfk($id)
    {
        ...
        // $regid = Crypt::decryptString($id);
        $regid = "";
            $pdf = PDF::loadview('master.revisi.lpfk', compact('regid'));
        ...
        return view('master.revisi.lpfk', compact('regid'));
    ...
    public function reportskkt($id)
    {
        ...
        // $regid = Crypt::decryptString($id);
        $regid = "";
            $pdf = PDF::loadview('master.revisi.skkt', compact('regid'));
            $pdf->getDomPDF()->setHttpContext(
                stream_context_create([
                    'ssl' => [
                        'allow_self_signed'=> TRUE,
        ...
        return view('master.revisi.skkt', compact('regid'));
    ...
    public function reportspa($id)
    {
        ...
        // $regid = Crypt::decryptString($id);
        $regid = "";
            $pdf = PDF::loadview('master.revisi.spa', compact('regid'));
            $pdf->getDomPDF()->setHttpContext(
                stream_context_create([
                    'ssl' => [
                        'allow_self_signed'=> TRUE,
        ...
        return view('master.revisi.spa', compact('regid'));
    ...
    public function reportspkk($id)
    {
        ...
        // $regid = Crypt::decryptString($id);
        $regid = "";
            $pdf = PDF::loadview('master.revisi.spkk', compact('regid'));
            $pdf->getDomPDF()->setHttpContext(
                stream_context_create([
                    'ssl' => [
                        'allow_self_signed'=> TRUE,
        ...
        return view('master.revisi.spkk', compact('regid'));
    ...
    public function reportspm($id)
    {
        ...
        // $regid = Crypt::decryptString($id);
        $regid = "";
            $pdf = PDF::loadview('master.revisi.spm', compact('regid'));
            $pdf->getDomPDF()->setHttpContext(
                stream_context_create([
                    'ssl' => [
                        'allow_self_signed'=> TRUE,
        ...
        return view('master.revisi.spm', compact('regid'));
    ...
    public function doc($id)
    {
        ...
        return view('master.revisi.doc', compact('data','dokumen','vlevel','regid'));
    }
    public function upload(Request $request){
        ...
        return redirect()->intended('revisi/edit/'.$request->regid)->with('success', 'Data Berhasil dihapus');
    ...
    public function update(Request $request)
    {
        ...
        $susia      = app('App\Http\Controllers\Master\revisiController')->hitung_umur($stgllahir, $smulai);
        $sbulan     = app('App\Http\Controllers\Master\revisiController')->hitung_umur($stgllahir, $smulai, "bulan");
        $shari      = app('App\Http\Controllers\Master\revisiController')->hitung_umur($stgllahir, $smulai, "hari");
    //  $total_usia = ($susia + ($smasa / 12));
        
        // belum naik / reset bulan hari
        $sakhir     = date('Y-m-d', strtotime('+' . $smasa . 'months', strtotime($smulai)));
        $busia      = app('App\Http\Controllers\Master\revisiController')->hitung_umur($stgllahir, $sakhir, null, false);
        $bbulan     = app('App\Http\Controllers\Master\revisiController')->hitung_umur($stgllahir, $sakhir, "bulan", false);
        $bhari      = app('App\Http\Controllers\Master\revisiController')->hitung_umur($stgllahir, $sakhir, "hari", false);
       ...
       if ($srates == "") {
        ...
        // dd($hasil);
        return redirect()->intended('revisi')->with('success', 'Data Berhasil diubah');
    }
