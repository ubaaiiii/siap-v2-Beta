...
class SertifikatController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        // {
            ...
            $sqlq .= " AND asuransi LIKE '$cari' OR regid LIKE '$cari' OR nama LIKE '$cari' OR tgllahir LIKE '$cari' OR mulai LIKE '$cari' OR up LIKE '$cari' OR premi LIKE '$cari' OR pd.msdesc LIKE '$cari' OR masa LIKE '$cari' OR nopeserta LIKE '$cari' OR ab.msdesc LIKE '$cari' OR aa.policyno LIKE '$cari' OR aa.status LIKE '$cari' order by aa.regid desc ";
    ...
    public function getDesktop(Request $request)
    {
        if($request->ajax())
        {
            ...
            {
				/* $sqlq = "SELECT SQL_CALC_FOUND_ROWS ma.msdesc asuransi,regid,nama,tgllahir,mulai,up,premi,pd.msdesc produk,masa,nopeserta,ab.msdesc status,aa.policyno,aa.status status_code FROM ";
				$sqlq .= "(select * from tr_sppa where status IN ( '5', '6', '20', '73', '83' ) and asuransi IN (SELECT cabang FROM   ms_admin WHERE  level = 'insurance' AND    username = '$userid')) aa ";
				$sqlq .= "INNER JOIN (select msid,msdesc,mstype from ms_master where mstype='STREQ') ab ON aa.status = ab.msid AND ab.mstype = 'STREQ' " ;
				$sqlq .= "INNER JOIN (select msid,msdesc,mstype from ms_master where mstype='ASURANSI') ma ON aa.asuransi = ma.msid AND ma.mstype = 'ASURANSI' ";
				$sqlq .= "INNER JOIN (select msid,msdesc,mstype from ms_master where mstype='produk') pd ON pd.msid = aa.produk AND pd.mstype = 'produk' "; */
				$sqlq = "SELECT SQL_CALC_FOUND_ROWS ma.msdesc asuransi,regid,nama,tgllahir,mulai,up,premi,pd.msdesc produk,masa,nopeserta,ab.msdesc status,aa.policyno,aa.status status_code FROM ";
            ...
            $sqlq .= " order by aa.regid desc LIMIT 1000 ";
			
            $data = DB::select($sqlq);
            return DataTables::of($data)
                ->addColumn('action', function($data){
                    $button = '<a href="'.'sertifikat/view/'. Crypt::encryptString($data->regid).'"  class="btn btn-default btn-sm" style="display:inline !important;">View</a>&nbsp;';
                    return $button;
                })
                ->rawColumns(['action'])
                ->make(true);
    ...
    public function index()
    {
        ...
        // dd(DB::select("select * from tr_sppa where status IN ( '5', '6', '20', '73', '83' )"));
        $sqlq = "SELECT SQL_CALC_FOUND_ROWS ma.msdesc asuransi,regid,nama,tgllahir,mulai,up,premi,pd.msdesc produk,masa,nopeserta,ab.msdesc status,aa.policyno,aa.status status_code FROM ";
        $sqlq .= "(select * from tr_sppa where status IN ( '5', '6', '20', '73', '83' ) ) aa ";
        $sqlq .= "INNER JOIN (select msid,msdesc,mstype from ms_master where mstype='STREQ') ab ON aa.status = ab.msid AND ab.mstype = 'STREQ' " ;
        $sqlq .= "INNER JOIN (select msid,msdesc,mstype from ms_master where mstype='ASURANSI') ma ON aa.asuransi = ma.msid AND ma.mstype = 'ASURANSI' ";
        ...
        if($agent->isDesktop()){
            return view('master.sertifikat.index_desktop', compact('data','sertifikat'));
        }else{
            return view('master.sertifikat.index', compact('data','sertifikat'));
        }
    }
    public function view($id)
    { 
        $id = Crypt::decryptString($id);
        $sqle = " SELECT aa.regid,aa.nama,aa.noktp,aa.jkel,aa.pekerjaan,aa.cabang,aa.tgllahir,aa.mulai,aa.akhir,aa.masa,aa.up,aa.status,aa.createdt,aa.createby,aa.editdt,aa.editby,aa.validby,aa.validdt,aa.nopeserta,aa.usia,aa.premi,aa.epremi,aa.tpremi,aa.bunga,aa.tunggakan,aa.produk,aa.mitra,aa.comment,aa.asuransi,aa.policyno,ab.msdesc tstatus,ac.paiddt,aa.alamat,aa.hubungan_ahli_waris,aa.nama_ahli_waris,aa.notelp_ahli_waris FROM   tr_sppa aa LEFT JOIN ms_master ab ON aa.status = ab.msid AND ab.mstype = 'STREQ' LEFT JOIN tr_sppa_paid ac ON ac.regid = aa.regid WHERE aa.regid = '$id'";
        $produk = DB::select("select ms.msid comtabid,msdesc comtab_nm from ms_master ms where ms.mstype='produk' and ms.msid<>'ALL'  order by ms.mstype");
        ...
        return view('master.sertifikat.view',compact('data','produk','cabang','mitra','pekerjaan','asuransi','jkel','file','hubungan'));
    }
    ...
    {   
        ...
        $ms_insurance =DB::select("select * from ms_insurance");
        // dd($ms_insurance);
    ...
    {   
        ...
        $transaction = DB::select("SELECT * FROM tr_sppa a LEFT JOIN tr_billing b ON a.policyno = b.policyno INNER JOIN ms_master mp ON mp.msid = a.produk AND mp.mstype = 'produk' WHERE  a.policyno = '$spolicyno'");
