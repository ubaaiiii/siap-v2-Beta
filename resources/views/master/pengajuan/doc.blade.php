	                        ...
	                        <div class="card-body">
	                            <div class="row">
	                                ...
	                                <div class="col align-self-center ps-0">
										@if($dok->file !== NULL)
	                                    <p class="small mb-1"><a href="{{ url($dok->file.'/'.$dok->nama_file) }}" class="fw-medium">{{ $dok->msdesc }}</a> <span
	                                            class="text-muted">{{ $dok->tipe_file }}</span></p>
										@else
										<p class="small mb-1"><a href="#" class="fw-medium">{{ $dok->msdesc }}</a> <span
	                                            class="text-muted">{{ $dok->tipe_file }}</span></p>
										@endif
	                                    <p>{{ $dok->ukuran_file }} <small class="text-muted">{{ $dok->tglupload }}</small>
	                                    </p>
	                                </div>
	                                
	                                <div class="col-auto">
	                                	@if($dok->ukuran_file == null)
		                                	@if($vlevel != 'insurance')
			                                ...
			                                @endif
			                                @if ($dok->tipe_file == 'jpg' || $dok->tipe_file == 'png' || $dok->tipe_file == 'jpeg')
							                ...
							                <div class="row h-100 mb-4">
							                	<div class="col-12">
							                        <div class="form-group form-floating  mb-3">
							                            <input type="file" class="form-control" name="upload" required placeholder="Name" id="upload">
							                            <label for="upload">upload</label>
							                        </div>
							                        <input type="hidden" name="jdoc" value="{{ $dok->msid }}">
